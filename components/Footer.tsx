...
export function Footer({ locale, dict }: { locale: Locale; dict: Dictionary }) {
  return <footer className="site-footer"><div className="container footer-grid">
    <div className="footer-brand"><Logo locale={locale} /><p>{company.tagline}<br />{company.country}</p></div>
  ...
  </div><div className="container footer-bottom"><p>© {new Date().getFullYear()} {company.name}. {dict.footer.rights}</p><p>{dict.footer.credit}</p></div></footer>;
}
