...
export function ContactPage({ locale, dict }: { locale: Locale; dict: Dictionary }) {
  return <><PageHero eyebrow={dict.contact.eyebrow} title={dict.contact.title} body={dict.contact.body} /><section className="section contact-page"><div className="container contact-grid"><div className="contact-card"><span><Phone /></span><p>{dict.contact.phone}</p><a href={company.phoneHref}>{company.phoneDisplay}</a></div><div className="contact-card"><span><Mail /></span><p>{dict.contact.email}</p><a href={`mailto:${company.email}`}>{company.email}</a></div><div className="contact-card contact-card-whatsapp"><span><MessageCircle /></span><a href={company.whatsapp} target="_blank" rel="noopener noreferrer">{dict.contact.whatsappAction}</a></div><div className="contact-card"><span><MapPin /></span><p>{dict.contact.areas}</p><strong>{dict.contact.areasValue}</strong></div></div><div className="container contact-social"><SocialLinks dict={dict} /></div><div className="container contact-map-wrap"><div className="contact-map-copy"><span><MapPin /></span><div><strong>{dict.contact.areasValue}</strong><small>{company.country}</small></div></div><iframe className="contact-map" title={dict.contact.mapTitle} src="https://www.google.com/maps?q=Paphos%20Limassol%20Cyprus&output=embed" loading="lazy" referrerPolicy="no-referrer-when-downgrade" /></div></section></>;
}
