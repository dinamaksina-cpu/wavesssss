...
function QuickBooking({ dict, locale }: { dict: Dictionary; locale: Locale }) {
  ...
  const [contact, setContact] = useState<Contact>(emptyContact); const [message, setMessage] = useState(""); const [error, setError] = useState("");
  function submit(event: FormEvent) {
    ...
    setMessage([handoff.quick.greeting, "", handoff.quick.intro, "", `${t.name}: ${clean.name}`, `${t.phone}: ${clean.phone}`, `${t.email}: ${clean.email}`, "", handoff.quick.closing].join("\n"));
  }
  if (message) return <WhatsAppReady locale={locale} message={message} />;
...
function QuoteCalculator({ dict, locale }: { dict: Dictionary; locale: Locale }) {
  ...
  const [contact, setContact] = useState<Contact>(emptyContact); const [error, setError] = useState(""); const [summary, setSummary] = useState(false); const [message, setMessage] = useState("");
  const update = (field: keyof typeof form, value: string) => { setForm({ ...form, [field]: value }); setSummary(false); setMessage(""); };
  ...
  function send() {
    ...
    setError("");
    const clean = validation.data;
    setMessage([handoff.quote.greeting, "", handoff.quote.intro, "", `${flow.name}: ${clean.name}`, `${flow.phone}: ${clean.phone}`, `${flow.email}: ${clean.email}`, ...rows.map(([label, value]) => `${label}: ${value}`), "", handoff.quote.closing].join("\n"));
  }
  if (message) return <WhatsAppReady locale={locale} message={message} />;
...
function WhatsAppReady({ locale, message }: { locale: Locale; message: string }) {
  const t = whatsappBookingTranslations[locale];
  return <section className="booking-card booking-success" aria-live="polite"><CheckCircle2 aria-hidden="true" /><h2>{t.readyTitle}</h2><p>{t.readyBody}</p><p className="form-note">{t.manualSendNote}</p><div className="quote-actions"><button className="button" type="button" onClick={() => continueToWhatsApp(message)}><MessageCircle size={17} />{t.continue}</button></div></section>;
}
